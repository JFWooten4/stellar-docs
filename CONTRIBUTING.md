...
- **Prefer the `README.mdx` convention over `_category_.yml` for category metadata.** A category's index `README.mdx` frontmatter drives its title, sidebar label, and position, so do as much as possible there. Add a `_category_.yml` file only when something genuinely can't be expressed in `README.mdx` — e.g., overriding `collapsible`/`collapsed`, or setting a category `description` (which isn't hoisted from the `README.mdx` frontmatter) — and include only those necessary keys.
