...
| Disk reads[^disk-reads]                               | 100 entries/200 KB |
...
[^disk-reads]: Most transactions don't perform any disk reads, as only archived entries and 'classic' entries (`G`-accounts, trustlines) are considered to be disk reads. Live smart contract state is in-memory and is not counted towards this limit.
...
These define contract data lifespans, also established through validator consensus.
