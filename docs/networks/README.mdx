...
Stellar has three networks: Mainnet (also called Pubnet or the Public Network), Testnet, and Futurenet.

- **Mainnet** is the network used by applications in production. It connects to real financial rails and requires XLM to cover minimum balances, transaction fees, and rent.
...
It is recommended that you have testing infrastructure that can repopulate Testnet and Futurenet with useful data after a reset. This will make testing more reliable and will help you scale your testing infrastructure to a private network if you choose to do so. For example, you may want to:
...
- Performing data analysis on a smaller, non-trivial data set compared to Mainnet.
...
- A testing infrastructure that requires more control over the test environment, such as:
  ...
  - The need to secure private or sensitive data (before launching on Mainnet). You can always run your own test network for use cases that don’t work well with SDF’s Testnet.
