...
Once the four transactions are run, you should see the proposed upgrade when running the `dumpproposedsettings` command (learn more in [step 4 above](#4-verify-proposed-upgrades)). If you don't, then either one or more of the transactions above failed during application, or the upgrade is invalid.
