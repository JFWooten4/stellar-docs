...
An indicative rate is an estimate that may change before the transaction is executed. A firm rate commits the
anchor to the quoted exchange rate until a specified expiration. The anchor must provide an ID and expiration if
the client requests a firm rate.
