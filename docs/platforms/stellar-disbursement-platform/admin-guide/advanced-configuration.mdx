...
2.  **Subdomain**: The prefix of the hostname (_e.g._, `tenant1` in `tenant1.sdp.stellar.org`).
...
Ensure your DNS provider has a wildcard A record (_e.g._, `*.sdp.stellar.org`) pointing to your Ingress Controller's Load Balancer IP.
...
| `RPC_REQUEST_AUTH_HEADER_KEY` | The HTTP header name for authenticating requests to a protected RPC server (_e.g._, `X-API-Key`). Optional. | - | No |
