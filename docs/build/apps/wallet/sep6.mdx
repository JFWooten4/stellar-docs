...
Let's add some KYC info for our account using sample data. Binary data (_e.g._ image data), needs to be sent in a separate field. The fields allowed to send to the anchor are described in [SEP-9].
