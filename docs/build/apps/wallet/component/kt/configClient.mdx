...
After the wallet class is no longer used, it's necessary to close all clients used by it. While in some applications it may not be required (_e.g._ the wallet lives for the whole lifetime of the application), in other cases it can be required. If your wallet class is short-lived, it's recommended to close client resources using a close function:
