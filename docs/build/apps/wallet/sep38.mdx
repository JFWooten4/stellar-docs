...
The [SEP-38] standard defines a way for anchors to provide quotes for the exchange of an off-chain asset and a different on-chain asset, and vice versa. An indicative quote is an estimate that may change before the exchange occurs. A firm quote commits the anchor to the quoted rate until the quote expires. The sections below explain when to use each type.
...
Now let's get indicative prices from the anchor in exchange for a given asset. The actual price will be calculated at conversion time once the anchor receives the funds from a user.
...
Now let's get a firm quote from the anchor. Unlike the indicative prices returned by the earlier endpoints, this quote is stored by the anchor and remains valid until it expires. We will show how to retrieve the quote again later.
