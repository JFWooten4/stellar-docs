...
On Stellar, MPP works with Soroban SAC ([Stellar Asset Contract](../../../tokens/stellar-asset-contract.mdx)) token transfers so that clients can pay for API requests natively, without an external facilitator. This makes it ideal for micropayments, AI agent-driven APIs, and payment-enabled applications.
