---
title: "Send and Receive Payments with Contract Accounts"
...
Learn more in the [Monitoring Payments using the Horizon API example](https://github.com/fazzatti/c-address-payment-examples#monitoring-payments).
...
Learn more in the [Monitoring Payments using the Stellar RPC example](https://github.com/fazzatti/c-address-payment-examples#using-the-stellar-rpc).
...
This approach is simpler but requires the use of an RPC and involves a slightly different transaction workflow than a traditional native operation. Learn more in the [Sending Payments through the SAC using the Stellar RPC example](https://github.com/fazzatti/c-address-payment-examples#sending-payments-through-the-sac).
...
This process is fully handled by the SDK in a very simple step by using the ‘_prepareTransaction_’ function of the RPC server client. It automatically simulates and provides an updated transaction object. Learn more in the [RPC transaction demo](https://github.com/fazzatti/c-address-payment-examples/blob/0e1c021e5afbefd5946878d53934b62cf5a8bb68/src/core/rpc-transaction.ts#L43).
...
When sending a ‘transfer’ via the Horizon API, this approach introduces a few extra steps when compared to the use of an RPC. Learn more in the [Sending Payments through the SAC using the Horizon API example](https://github.com/fazzatti/c-address-payment-examples#using-the-horizon-api-1).
...
The **auth entries** can be easily predicted and assembled based on the configuration. Assuming the ‘sender’ account is the same as the ‘source’ account of the transaction, a source-account credential needs to be added to the operation, indicating this invocation will be used the same envelope signature to authorize the ‘transfer’. Learn more in the [‘assembleSourceAuthEntry’ demo function](https://github.com/fazzatti/c-address-payment-examples/blob/main/src/core/assemble-source-auth.ts).
...
The amount of resources consumed shouldn't vary much with the exception of some edge cases such as the transaction involving archived entries or a custom authorization for a smart wallet as the sender. Learn more about manually assembling the Soroban Data in the [‘getSorobanData’ demo function](https://github.com/fazzatti/c-address-payment-examples/blob/main/src/core/get-sorobandata.ts).
