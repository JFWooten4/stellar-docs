...
func main() {
  ...
  sourceAccount, _ := rpcClient.LoadAccount(ctx, sourceKeypair.Address())
...
func main() {
  ...
  var channelAccounts []txnbuild.Account
  for _, kp := range channelKeypairs {
    account, err := rpcClient.LoadAccount(ctx, kp.Address())
    check(err)
	...
	for recipientIndex < len(allRecipients) {
		for _, channelData := range channelAccountsTracker {
			...
			if channelData["state"] == "available" {
				...
				sourceAccount := channelData["account"].(txnbuild.Account)
