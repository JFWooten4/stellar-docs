...
The default string format for `Address` is a [string key, or StrKey](../../../learn/fundamentals/data-format/strkey.mdx). StrKeys for externally owned accounts begin with `G`, while StrKeys for contracts begin with `C`.
