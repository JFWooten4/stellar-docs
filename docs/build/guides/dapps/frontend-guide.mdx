...
- Mainnet (also called the Public Network): This is the Stellar network where real transactions take place.
- Testnet: This is a test environment for developers to test their applications without using real lumens.
- Futurenet: This is a network for testing new features before they are deployed to Mainnet.

For this guide, we'll be using Testnet to avoid using real lumens during development. Read more about the [Stellar networks here](../../../networks/README.mdx).
...
We also create a transaction builder instance with the appropriate network passphrase. The `BASE_FEE` is the minimum fee required for a transaction on the Stellar network, while `Networks.TESTNET` is the network passphrase for Stellar Testnet. When using Mainnet, you can replace `TESTNET` with `PUBLIC`, or use `FUTURENET` for Futurenet.
