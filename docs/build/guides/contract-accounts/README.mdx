...
- You must interoperate with systems/tools that expect memos (for example, some exchanges require payments with memos from a G-address; contract account payments go through the [Stellar Asset Contract](../../../tokens/stellar-asset-contract.mdx) as muxed transfers, so transfers from contracts are not supported by exchanges today).
