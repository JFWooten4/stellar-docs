...
The SacDeployer contract will be responsible for deploying the [Stellar Asset Contract](../../../tokens/stellar-asset-contract.mdx). Here is the code for the SacDeployer contract:
...
- `deploy_sac` function:
  - `env.deployer().with_stellar_asset(serialized_asset)`: creates a deployer configured to deploy a [Stellar Asset Contract](../../../tokens/stellar-asset-contract.mdx) using the provided serialized asset.
...
By following this guide, you’ve successfully deployed a [Stellar Asset Contract](../../../tokens/stellar-asset-contract.mdx) from within another contract using the Soroban Rust SDK. This approach enables smart contracts to handle SAC deployments dynamically, providing flexibility for various use cases in the Stellar ecosystem.
