...
Previously, the only way to limit data storage was by limiting the temporal range of history via rolling retention (_e.g._ the last 30 days). The filtering feature allows users to store a longer historical timeframe in the Horizon database for only whitelisted assets, accounts, and their related historical entities (transactions, operations, trades, etc.).
