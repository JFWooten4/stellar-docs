...
Ready to get started? The [Getting Started](./getting-started/README.mdx) section of the Quickstart documentation will show how to run Quickstart, but there are some configurations to decide on first. Most importantly, the network mode Quickstart will run in (pubnet, testnet, futurenet, local). Read about the modes in the [Network Modes](./network-modes.mdx) section.

:::info

Quickstart can be deployed using the [Getting Started](./getting-started/README.mdx) guide, but the configurations used in the guide may not be ideal for your use case, so please familiarize yourself with the options to configure Quickstart to fit your use case.
