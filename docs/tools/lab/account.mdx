...
1. Make sure you are on the Stellar network for which you want to generate the keypair. You can see the current network in the upper right corner of the page. Click on dropdown to change the network (don’t forget to provide any necessary information). We will use Testnet for this example.
...
The Friendbot can be used for new account or accounts with balance under a starting balance of 10,000 XLM. Click the "Fund account with Friendbot" button again to see what happens. It’s the Lab, so feel free to experiment (as long as you are on Testnet or Futurenet)!
...
3. Once you enter the public key, click the "Get lumens" button to fund the account with 10,000 XLM on Testnet or Futurenet.
