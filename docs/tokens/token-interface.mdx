...
Token contracts, including the [Stellar Asset Contract](./stellar-asset-contract.mdx) and example token implementations expose the following common interface.
...
Code written against the shapes above will therefore mis-parse an event emitted by a [Stellar Asset Contract](./stellar-asset-contract.mdx), reading the asset identifier as a missing field or ignoring it entirely.

[CAP-0067](https://github.com/stellar/stellar-protocol/blob/master/core/cap-0067.md) is the normative source for the [Stellar Asset Contract](./stellar-asset-contract.mdx) shapes below, and for the unification of classic events described further down.

| Event | SEP-41 contract token | [Stellar Asset Contract](./stellar-asset-contract.mdx) |
| --- | --- | --- |
...
Since protocol 23, classic operations emit these same events, published under the asset's [Stellar Asset Contract](./stellar-asset-contract.mdx) address whether or not that contract has been deployed — see [Tracking the movement of value](../learn/fundamentals/stellar-data-structures/events.mdx#tracking-the-movement-of-value).
