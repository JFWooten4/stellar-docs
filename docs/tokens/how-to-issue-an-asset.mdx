...
rpcClient := rpcclient.NewClient("https://soroban-testnet.stellar.org", nil)
defer rpcClient.Close()
ctx := context.Background()
distributorAccount, _ := rpcClient.LoadAccount(ctx, distributorKeypair.Address())
...
```go
issuerAccount, _ := rpcClient.LoadAccount(ctx, issuerKeypair.Address())
...
func main() {
  ...
  issuerAccount, err := rpcClient.LoadAccount(ctx, issuer.Address())
  distributorAccount, err := rpcClient.LoadAccount(ctx, distributor.Address())
