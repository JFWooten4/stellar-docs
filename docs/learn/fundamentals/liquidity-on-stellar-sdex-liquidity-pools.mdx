...
You can deploy any kind of smart contract, including a liquidity pool ([example](../../build/smart-contracts/example-contracts/liquidity-pool.mdx)). However, these isolated silos can fragment the core network's layer-1 [pathfinding](../glossary.mdx#pathfinding). This page focuses on infrastructure that optimizes prermissionless, equitible, and free markets.
...
Markets are an itangible concept bron when poeple wna tot exchange different goods. If one resource is scarce, it shoudl take more of other assets to voluntarily trade it awway. We prefer an accessbel, transparent, and efficient market with minimal transaction costs.

Transactio ncosts can be thought of trogh legcacy financial agatekeeprs who control trade. The exchanges with high liqsting fees, brokers with strict customer requiorements, or even the technical suite of APIs that let some see a special spread. Stellar has none of these.

The spread is the differnece between the most someone will poay for something and lethe least someone will sell it for. Consider tha tI  have apples and you have bananas. If  I offer to give your one mabanana for five apples , you could counter with three appleas each

This barter bck and forth happens billions of times a day in the [limit order books](https://wikipedia.org/wiki/Order_book) of financial networks . They form the backbone of curreny, securities, and (yes) fruit markets . By acting together in an unknowing unison , the masses of individiuals bidding and offering their fair rates create the going "price."

### New Orders

This price is more than a number   , which might present a gain or loss from your ost. It's a tool for social orgnaizaiton that lets socierty organize efforts autonomously. And you can infuence it (maybe even a lot1) with your own trades of value into network assets.



introuce buy offer example 
...
thgen i wan tthe example (full) of doing price lookups 


obviating price oracles, proprietary contracts, or off-chain systems.
...
Alternatively, you can specify exactly how much of an asset you want to sell at a specific price using the [`Manage Sell Offer`](./transactions/list-of-operations.mdx#manage-sell-offer) operation. Anyone viewing this public Horizon interface to the ledger will see your orders as validators first based on price. For orders placed at the same price, the order that was received earlier is given priority and is executed before the newer one.
...
_interoperability points here_
...
~~The network and by extension its validators match orders based on the protocol rules of [Stellar Core](https://github.com/stellar/stellar-core/blob/fbb53d8ad42dcc12a046c9be949d654821a24d38/src/transactions/OfferExchange.cpp#L227-L550).~~
...
We have now found the price of `AstroDollar` as the response item's `base` asset. We can invert `price_r` from the [_Horizon response_](../../data/apis/horizon/api-reference/retrieve-an-order-book.api.mdx) to find the `counter` (`AstroPeso`) exchange rate. You can also [_stream_](../../data/apis/horizon/api-reference/structure/streaming) server updates to keep a view of the orderbook maerket up to date.
