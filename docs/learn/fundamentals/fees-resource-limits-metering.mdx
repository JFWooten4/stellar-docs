...
The parameters for each model, `a` and `b`, are calibrated and fitted offline against inputs of various sizes. The model cost parameters are stored in [network configuration entries](./stellar-data-structures/ledgers/entries.mdx#config-setting) and can be updated when validators agree on a [network upgrade](./stellar-data-structures/ledgers/headers.mdx#upgrades).
