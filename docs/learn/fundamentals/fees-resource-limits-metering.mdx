...
- **Ledger space rent:** the payment for the ledger entry TTL extensions (_i.e._, rent payments) and rent payments for increasing ledger entry size. Refer to the [state archival](../fundamentals/contract-development/storage/state-archival.mdx) section for more information about smart contract rent.
...
2. Ensure each component cost increases at most linearly (_i.e._, constant or linear) with respect to its input. That is, `y = a + bx`, where `y` is the cost output, `x` is the input, and `a` & `b` are the constant and linear model parameters, respectively.
