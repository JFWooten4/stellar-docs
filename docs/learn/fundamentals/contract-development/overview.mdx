...
| **Use Cases** | Time-bounded or easily re-creatable data (_e.g._ price oracles, signatures). Not suitable for correctness checks | Long-term user data that must persist beyond TTL (_e.g._ balances) | Shared contract state tied to contract instance (_e.g._ admin accounts, contract metadata) |
...
To add metadata to Contract Meta, please use the Stellar CLI command `stellar contract build --meta <meta>`, or use the Rust SDK (_e.g._ `contractmeta!(key="name", val="Defi Swap Contract")`).
