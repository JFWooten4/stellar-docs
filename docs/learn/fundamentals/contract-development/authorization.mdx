...
- If the `Address` data in this contract is being modified in a way that's not strictly beneficial to the user, then `require_auth` is probably needed (_e.g._ reducing the user's token balance needs to be authorized, while increasing it doesn't need to be authorized)
- If a contract calls another contract that will call `require_auth` for the `Address` (_e.g._ `token.xfer`), then adding `require_auth` in the caller would ensure that the authorization for the inner call can't be reused outside of your contract. For example, if you want to do something positive for the user, but only when they have transferred some token to your contract, then the contract call itself should `require_auth`.
