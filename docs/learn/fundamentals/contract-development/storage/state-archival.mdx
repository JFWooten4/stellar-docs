...
- Suitable for time-bounded data (_i.e._ price oracles, signatures, etc.) and easily recreateable data.
...
- Suitable for "shared" contract state that cannot be `Temporary` (_i.e._ admin accounts, contract metadata, etc.).
...
- Unlimited amount of storage.
- Suitable for user data that cannot be `Temporary` (_i.e._ balances).
...
Each `ContractData` and `ContractCode` entry has a `liveUntilLedger` field stored in its `LedgerEntry`. The entry is no longer live (_i.e._ either archived or deleted depending on storage type) when `current_ledger > liveUntilLedger`.
...
2. **Prepare your operation**. This means describing the ledger entries within the corresponding operation (_i.e._, `ExtendFootprintTTLOp` or `RestoreFootprintOp`) and its ledger footprint (the `SorobanTransactionData` field), then simulating it to fill out fee and resource usage information (when restoring, you usually have simulation results already).
