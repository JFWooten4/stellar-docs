...
For example, calling the native asset contract on Testnet:
...
Like the CLI, this will fetch the contract from Testnet and parse the contract's types/spec. It will auto-generate a class that allows you to ergonomically call the contract's methods:
