...
Stellar has three networks: Mainnet (also called Pubnet or the Public Network), Testnet, and Futurenet. Mainnet is used by applications in production. Testnet is a smaller, free-to-use network maintained by SDF that functions like Mainnet but doesn’t connect to real money and is the best place for developers to test their applications. Futurenet is a dev network you can use to test more bleeding-edge features.
