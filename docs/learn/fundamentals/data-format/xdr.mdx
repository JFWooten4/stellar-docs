...
Learn more about the full set of options for constructing a `Uint64` in the [`UnsignedHyper` TypeScript definition](https://github.com/stellar/js-stellar-base/blob/master/types/curr.d.ts#L127-L136). In fact, this file should be your primary guide to navigating the XDR if you're using JavaScript.

Other languages, such as Python (learn more in the [`stellar-sdk` `uint64` documentation](https://stellar-sdk.readthedocs.io/en/stable/_modules/stellar_sdk/xdr/uint64.html)), support arbitrarily-sized integers by default, so there are no such variations:
...
Learn more in the corresponding documentation for [Python](https://stellar-sdk.readthedocs.io/en/stable/api.html#scaddress) and [JavaScript](https://github.com/stellar/js-stellar-base/blob/master/types/curr.d.ts#L13881-L13886).
