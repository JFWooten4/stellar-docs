...
| `PAYMENT_NO_DESTINATION` | -5 | The receiving account does not exist. Note that this error will **not** be returned if the receiving account is the issuer of `asset` (_i.e._, when the asset is being _burned_). |
...
The Asset parameter is of the `TrustLineAsset` type. If you are modifying a trustline to a regular asset (_i.e._ one in a Code:Issuer format), this is equivalent to the Asset type. If you are modifying a trustline to a pool share, however, this is composed of the liquidity pool's unique ID.
